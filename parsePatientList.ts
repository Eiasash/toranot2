...
function parsePatientLine(
  ...
  // First token: room/bed code (supports 49-3, 58/2, ניטור-1, 352/, etc.)
  ...
  const ROOM_PATTERN =
    /^(?:\d{1,3}[-/]\d{1,2}|\d{1,4}\/|\d{1,4}|ניטור-?\d{1,2}|חדר-?\d{1,3})$/;

  if (ROOM_PATTERN.test(roomToken)) {
    room = roomToken;
    idx = 1;
  }
  ...
  for (const part of extraParts) {
    ...
    const isTask = /(?:בדיק|ב"ד|CT|US|צילום|דימות|\bBS\b|Bladder\s*Scan|לתת |להזמין|לבצע|למדוד|לשלוח|טיפול|ניקוז|עירוי|ייעוץ|שיחה|א\.?ק\.?ג)/i.test(part);
    if (isTask) {
      tasks.push({
        id: generateId("task-"),
        text: part,
        ...
        confidence: 0.8,
        category: /\bCT\b|\bUS\b|דימות|צילום/i.test(part)
          ? "imaging"
          : /ב"ד|בדיקת דם|מעבדה/i.test(part)
            ? "labs"
            : /\bBS\b|Bladder\s*Scan|קטטר|פולי/i.test(part)
              ? "procedure"
              : /שחרור|מכתב שחרור|סיכום/i.test(part)
                ? "discharge"
                : /ייעוץ|שיחה/i.test(part)
                  ? "consult"
                  : undefined,
      });
    } else {
